...
class CommentList extends Component {
    ...
    render() {
        ...
        const listView = list.map((doc) => {
            // const datetime = item.createdAt ? new Date(item.createdAt.seconds * 1000).toISOString().substring(0, 10) : "";
            const item = doc.data();
            return <CommentItem
                key={item.id}
                id={item.id}
        ...
        return (
            <div>
                ...
                {listView}
                <Button loading={isLoading} onClick={this.onLoadMore}>댓글 더 불러오기</Button>
            </div>
        )
    }
}
